...
export default function Orders(){
    ...
    const [ getOrders, { isLoading } ] = orderApi.useGetAllMutation()    
    const { orders, total } = useSelector(s => s?.orders)    
    ...
    return(
        <Box safeArea flex={1} variant="wrapper">
            ...
            <HStack py={4} px={4} space={4} alignItems="center" borderBottomWidth={1} borderColor="gray.200">
                ...
                <Box flex={1}>
                    <Heading m={0} size="sm">{`${toTitleCase(filter?.status || '')} Orders`}</Heading>
                </Box>
                <Box>
                    <Menu shadow="tabs" placement="top right" trigger={MenuTrigger}>
                        <Menu.Item onPress={() => setFilter({...filter, status: null})}>All Orders</Menu.Item>
                        <Menu.Item onPress={() => setFilter({...filter, status: 'PENDING'})}>Pending</Menu.Item>
                        <Menu.Item onPress={() => setFilter({...filter, status: 'APPROVED'})}>Approved</Menu.Item>
                        <Menu.Item onPress={() => setFilter({...filter, status: 'CANCELLED'})}>Cancelled</Menu.Item>
                        <Menu.Item onPress={() => setFilter({...filter, status: 'ON-HOLD'})}>On Hold</Menu.Item>                        
                    </Menu>
                </Box>
            </HStack>   
            ...
            <Box flex={1}>
            {
                ...
                <FlatList                     
                    ...
                    renderItem={({item}) => (
                        <OrderItem onPress={() =>navigate('OrderDetails', { id: item?._id})} item={item} />
                    )}
                />                                     
            }
            {                            
